// Command CloseApproachData-cli is a CLI + REPL for the CloseApproachData SDK,
...
const prompt = "close-approach-data"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "cadapi"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CLOSE_APPROACH_DATA_APIKEY (api key), CLOSE_APPROACH_DATA_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CLOSE_APPROACH_DATA_APIKEY carries the API key and
	// CLOSE_APPROACH_DATA_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CLOSE_APPROACH_DATA_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CLOSE_APPROACH_DATA_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCloseApproachDataSDK(opts)
