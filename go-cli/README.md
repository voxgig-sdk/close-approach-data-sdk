# close-approach-data-cli

boru-driven command-line client **and** interactive REPL for the CloseApproachData
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/close-approach-data-cli)
make build

# 2. See usage (words, entities, env vars)
./close-approach-data-cli --help

# 3. Provide credentials once, via the environment
export CLOSE_APPROACH_DATA_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./close-approach-data-cli list cadapi

# 5. Override the API base URL for a single call
CLOSE_APPROACH_DATA_BASE=https://api.example.com ./close-approach-data-cli list cadapi

# 6. No arguments -> interactive REPL
./close-approach-data-cli
close-approach-data> list cadapi
close-approach-data> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/close-approach-data-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/close-approach-data-cli list cadapi
   ```
...
Run with no arguments to open a REPL (prompt `close-approach-data>`). Each line is
evaluated as its own boru expression:

```text
$ ./close-approach-data-cli
close-approach-data> list cadapi
close-approach-data> /help
close-approach-data> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `cadapi`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/close-approach-data-cli`. |
...
(`./close-approach-data-cli <expr>`) and interactively (the REPL), and expressions compose the
