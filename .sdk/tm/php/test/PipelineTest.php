...
// CloseApproachData SDK pipeline test
//
...
{
    private static function utility(): CloseApproachDataUtility
    {
        return new CloseApproachDataUtility();
    }
    ...
    private static function ctx(array $over = []): CloseApproachDataContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CloseApproachDataContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CloseApproachDataOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CloseApproachDataError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CloseApproachDataOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CloseApproachDataMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CloseApproachDataOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CloseApproachDataMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CloseApproachDataOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CloseApproachDataMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CloseApproachDataSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CloseApproachDataMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CloseApproachDataResponse([]),
            'result' => new CloseApproachDataResult([]),
        ]);
        [, $err] = CloseApproachDataMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CloseApproachDataSpec([]),
            'response' => null,
            'result' => new CloseApproachDataResult([]),
        ]);
        [, $err] = CloseApproachDataMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CloseApproachDataSpec(['step' => 's']),
            'response' => new CloseApproachDataResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CloseApproachDataResult([]),
        ]);
        [, $err] = CloseApproachDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CloseApproachDataSpec(['step' => 's']),
            'response' => new CloseApproachDataResponse(self::resp(200, ['v' => 1])),
            'result' => new CloseApproachDataResult([]),
        ]);
        [, $err] = CloseApproachDataMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CloseApproachDataSpec(['step' => 's']),
            'response' => new CloseApproachDataResponse(self::resp(200, ['v' => 2])),
            'result' => new CloseApproachDataResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CloseApproachDataMakeResponse::call($ctx);
    ...
    {
        $preset = new CloseApproachDataResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CloseApproachDataSpec([]),
            'response' => new CloseApproachDataResponse([]),
            'result' => new CloseApproachDataResult([]),
        ...
        [$got, $err] = CloseApproachDataMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CloseApproachDataResult([])]);
        [, $err] = CloseApproachDataMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CloseApproachDataSpec([]), 'result' => null]);
        [, $err] = CloseApproachDataMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CloseApproachDataSpec(['step' => 's']),
            'result' => new CloseApproachDataResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CloseApproachDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CloseApproachDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CloseApproachDataSpec(['step' => 's']),
            'result' => new CloseApproachDataResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CloseApproachDataOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CloseApproachDataMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CloseApproachDataResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CloseApproachDataSpec([]),
            'result' => new CloseApproachDataResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CloseApproachDataMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CloseApproachDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CloseApproachDataError('boom', 'boom');
        $utility->fetcher = function (CloseApproachDataContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CloseApproachDataSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CloseApproachDataMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CloseApproachDataContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CloseApproachDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CloseApproachDataResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CloseApproachDataMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CloseApproachDataContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CloseApproachDataSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CloseApproachDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CloseApproachDataResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CloseApproachDataSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CloseApproachDataMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CloseApproachDataMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CloseApproachDataMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CloseApproachDataResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CloseApproachDataDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CloseApproachDataResult(['ok' => false])]);
        $this->expectException(CloseApproachDataError::class);
        CloseApproachDataDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CloseApproachDataResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CloseApproachDataMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CloseApproachDataMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CloseApproachDataBaseFeature();
        $b = new CloseApproachDataBaseFeature();
        CloseApproachDataFeatureAdd::call($ctx, $a);
        CloseApproachDataFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CloseApproachDataBaseFeature
    {
        $f = new CloseApproachDataBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CloseApproachDataFeatureAdd::call($ctx, self::named_feature('a'));
        CloseApproachDataFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CloseApproachDataFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CloseApproachDataFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CloseApproachDataFeatureAdd::call($ctx, $replace);
        ...
        CloseApproachDataFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CloseApproachDataContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CloseApproachDataSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CloseApproachDataPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CloseApproachDataPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CloseApproachDataResponse(['headers' => null]),
            'result' => new CloseApproachDataResult([]),
        ]);
        CloseApproachDataResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CloseApproachDataResultBody::call($ctx);
