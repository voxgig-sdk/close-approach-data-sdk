...
class CloseApproachDataMakeError
{
    public static function call(?CloseApproachDataContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new CloseApproachDataContext([], null);
        }
        $op = $ctx->op ?? new CloseApproachDataOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new CloseApproachDataResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof CloseApproachDataError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new CloseApproachDataError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof CloseApproachDataError) {
