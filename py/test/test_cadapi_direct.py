...
class TestCadapiDirect:

    def test_should_direct_list_cadapi(self):
        setup = _cadapi_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-cadapi", "live" if setup["live"] else "unit")
        if _skip:
...
def _cadapi_direct_setup(mockres):
