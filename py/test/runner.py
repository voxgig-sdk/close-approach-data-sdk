# CloseApproachData SDK test runner
...
class CloseApproachDataTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CloseApproachDataTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CloseApproachDataTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CloseApproachDataTestRunner.getenv("CLOSE_APPROACH_DATA_TEST_LIVE")
        override = CloseApproachDataTestRunner.getenv("CLOSE_APPROACH_DATA_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CloseApproachDataTestRunner.getenv(key)
        ...
        explain = CloseApproachDataTestRunner.getenv("CLOSE_APPROACH_DATA_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CloseApproachDataTestRunner._test_control is not None:
            return CloseApproachDataTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CloseApproachDataTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CloseApproachDataTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CloseApproachDataTestRunner.load_test_control()
...
def load_env_local():
    CloseApproachDataTestRunner.load_env_local()


def env_override(m):
    return CloseApproachDataTestRunner.env_override(m)


def entity_data(v):
    return CloseApproachDataTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CloseApproachDataTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CloseApproachDataTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CloseApproachDataTestRunner.load_test_control()


def live_client_options():
    return CloseApproachDataTestRunner.live_client_options()


def live_delay_ms():
    return CloseApproachDataTestRunner.live_delay_ms()
