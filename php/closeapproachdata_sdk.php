...
class CloseApproachDataSDK
{
    ...
    public function __construct(array $options = [])
    {
        ...
        $feature_opts = CloseApproachDataHelpers::to_map(Struct::getprop($this->options, "feature"));
        if ($feature_opts) {
            $featureorder = Struct::getpath($this->options, "__derived__.featureorder");
            if (is_array($featureorder)) {
                foreach ($featureorder as $fname) {
                    $fopts = CloseApproachDataHelpers::to_map($feature_opts[$fname] ?? null);
                    if ($fopts && isset($fopts["active"]) && $fopts["active"] === true) {
                        ...
                        if (!CloseApproachDataFeatures::has_feature($fname)) {
                        ...
                        ($utility->feature_add)($this->_rootctx, CloseApproachDataFeatures::make_feature($fname));
                    }
                }
            }
        }
    ...
    {
        return CloseApproachDataUtility::copy($this->_utility);
    }
    ...
    public function prepare(array $fetchargs = []): mixed
    {
        ...
        $ctrl = CloseApproachDataHelpers::to_map(Struct::getprop($fetchargs, "ctrl")) ?? [];
        ...
        $params = CloseApproachDataHelpers::to_map(Struct::getprop($fetchargs, "params")) ?? [];
        $query = CloseApproachDataHelpers::to_map(Struct::getprop($fetchargs, "query")) ?? [];
    ...
    private function op_denied(string $op): array
    {
        ...
        return [
            "ok" => false,
            "err" => new CloseApproachDataError($op . "_allow",
                "CloseApproachDataSDK: " . $op . ": operation not allowed by" .
    ...
    private function raw_request(array $fetchargs = []): mixed
    {
        ...
        if (is_array($fetched)) {
            $status = CloseApproachDataHelpers::to_int(Struct::getprop($fetched, "status"));
    ...
    public function graphql(string $query, ?array $variables = null, ?array $ctrl = null): mixed
    {
        ...
        if (is_array($errors) && 0 < count($errors)) {
            ...
            $res["ok"] = false;
            $res["err"] = new CloseApproachDataError("graphql_error",
                "CloseApproachDataSDK: graphql: " . $msg);
            $res["graphql"] = $errors;
        }
    ...
    public function Cadapi($data = null)
    {
        ...
        if ($data === null) {
            if ($this->_cadapi === null) {
                $this->_cadapi = new CadapiEntity($this, null);
            }
            return $this->_cadapi;
        }
        return new CadapiEntity($this, $data);
    }
    ...
    public static function test(?array $testopts = null, ?array $sdkopts = null): self
    {
        ...
        $sdk = new CloseApproachDataSDK($sdkopts);
        $sdk->mode = "test";
        return $sdk;
    }
}
