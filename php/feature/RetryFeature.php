...
// CloseApproachData SDK retry feature
...
class CloseApproachDataRetryFeature extends CloseApproachDataBaseFeature
{
    ...
    public function init(CloseApproachDataContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CloseApproachDataContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CloseApproachDataContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CloseApproachDataContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
