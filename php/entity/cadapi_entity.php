...
// CloseApproachData SDK Cadapi entity
...
class CadapiEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new CadapiEntity($this->_client, $opts);
    }

    /**
     * @param Cadapi|array $args Cadapi data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CloseApproachDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Cadapi|array The current Cadapi data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Cadapi fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CloseApproachDataHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Cadapi fields).
     */
    ...
    /**
     * List Cadapi items matching the given filter.
     *
     * @param CadapiListMatch|array|null $reqmatch Match filter (any subset
     *   of Cadapi fields) as an assoc-array; CadapiListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Cadapi[]|array A list of Cadapi items as assoc-arrays at
     *   the SDK boundary; throws CloseApproachDataError on failure (item-5 convention).
     */
