...
// CloseApproachData SDK feature test
//
...
class FtCtrl extends CloseApproachDataControl
{
...
class FtClient
{
    ...
    private CloseApproachDataUtility $utility;

    public function __construct(CloseApproachDataUtility $utility, array $options)
    {
    ...
    public function get_utility(): CloseApproachDataUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CloseApproachDataContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CloseApproachDataContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CloseApproachDataContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CloseApproachDataConfig::make_feature($name);
    ...
    private static function buildUrl(CloseApproachDataSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CloseApproachDataError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CloseApproachDataResponse($fetched);
        }
    ...
    private function fail(CloseApproachDataContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CloseApproachDataContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
