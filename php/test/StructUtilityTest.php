...
// CloseApproachData SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = CloseApproachDataOmni::makeStructRunner(
    ...
    {
        return CloseApproachDataOmni::objmodel(CloseApproachDataOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return CloseApproachDataOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return CloseApproachDataOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(CloseApproachDataOmni::structfix($now, Struct::undef()))
                        !== json_encode(CloseApproachDataOmni::structfix($val, Struct::undef()))) {
