...
//     (\`new CloseApproachDataSDK(...)\` / \`CloseApproachDataSDK.test(...)\` become
//     \`CloseApproachDataSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CloseApproachDataSDK'
...
const TEST_SEED = {"entity":{"cadapi":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CloseApproachDataSDK(...)\` and any
// \`CloseApproachDataSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CloseApproachDataSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CloseApproachDataSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
