...
[MCP](https://modelcontextprotocol.io) server exposing the CloseApproachData SDK as
two agent tools — `close-approach-data_list` and `close-approach-data_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/close-approach-data-mcp)
make build

# 2. Provide credentials via the environment
export CLOSE_APPROACH_DATA_APIKEY=sk_live_xxx
...
```jsonc
// close-approach-data_list: first page of records
{ "entity": "cadapi" }
{ "entity": "cadapi", "query": { } }

// close-approach-data_load: one record by id
{ "entity": "cadapi", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/close-approach-data-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user close-approach-data \
     -- "$PWD"/dist/*/close-approach-data-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `close-approach-data_list` and `close-approach-data_load` tools now appear
   in new sessions. Ask the agent to *"list cadapi using close-approach-data"*
   and it calls `close-approach-data_list` with `{"entity":"cadapi"}`.
...
```sh
export CLOSE_APPROACH_DATA_APIKEY=sk_live_xxx            # API key
export CLOSE_APPROACH_DATA_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `close-approach-data_list` tool
...
### Call the `close-approach-data_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `close-approach-data_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `close-approach-data_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `CLOSE_APPROACH_DATA_APIKEY` | API key sent with every request. |
| `CLOSE_APPROACH_DATA_BASE` | Optional override of the API base URL. |
