...
require_relative 'CloseApproachData_types'
...
class CloseApproachDataSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CloseApproachDataHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CloseApproachDataHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CloseApproachDataFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CloseApproachDataUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CloseApproachDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CloseApproachDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CloseApproachDataHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CloseApproachDataError.new(
        "#{op}_allow",
  ...
  def raw_request(fetchargs = {})
    ...
    rescue CloseApproachDataError => err
    ...
    if fetched.is_a?(Hash)
      status = CloseApproachDataHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CloseApproachDataError.new(
        "graphql_error", "CloseApproachDataSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CloseApproachDataSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
