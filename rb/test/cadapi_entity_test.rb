...
class CadapiEntityTest < Minitest::Test
  ...
  def test_stream
    ...
    base = CloseApproachDataSDK.test(seed, nil)
    seen = base.Cadapi(nil).stream("list", nil, nil).to_a
    ...
    if cfg["feature"].is_a?(Hash) && cfg["feature"].key?("streaming")
      ...
      got = []
      sdk.Cadapi(nil).stream("list", nil, nil).each do |item|
        if item.is_a?(Array)
          got.concat(item)
        else
          got << item
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CLOSE_APPROACH_DATA_TEST_CADAPI_ENTID JSON to run live"
      return
    end
    client = setup[:client]
...
def cadapi_basic_setup(extra)
  ...
  entid_env_raw = ENV["CLOSE_APPROACH_DATA_TEST_CADAPI_ENTID"]
