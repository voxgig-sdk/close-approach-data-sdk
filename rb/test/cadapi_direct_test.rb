...
class CadapiDirectTest < Minitest::Test
  def test_direct_list_cadapi
    setup = cadapi_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-cadapi", setup[:live] ? "live" : "unit")
    if _should_skip
...
def cadapi_direct_setup(mockres)
  ...
  if live
    ...
    client = CloseApproachDataSDK.new(merged_opts)
    return {
      client: client,
      calls: calls,
      live: true,
