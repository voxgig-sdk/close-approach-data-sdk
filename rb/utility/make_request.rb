...
module CloseApproachDataUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CloseApproachDataResponse.new({})
    ...
    if fetch_err
      ...
      response = CloseApproachDataResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CloseApproachDataResponse.new(fetched)
    else
