...
class CadapiEntity
  ...
  def make
    opts = @_entopts.dup
    CadapiEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CloseApproachDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Cadapi, Hash] the current Cadapi data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CloseApproachDataHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Cadapi fields)
  def match_get
  ...
  #
  # @param reqmatch [CadapiListMatch, Hash, nil] match filter (any subset of
  #   Cadapi fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Cadapi>, Array] the matching Cadapi items; raises CloseApproachDataError on failure
