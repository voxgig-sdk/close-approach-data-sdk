...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("CLOSEAPPROACHDATA_TEST_LIVE") == "TRUE" ||
		os.Getenv("CLOSEAPPROACHDATA_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("CLOSEAPPROACHDATA_TEST_EXPLAIN"); explain != "" {
