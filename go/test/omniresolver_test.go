...
import (
	...
	omni "github.com/voxgig-sdk/close-approach-data-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CloseApproachDataError); is {
				out := map[string]any{
					"name":    "CloseApproachDataError",
...
func omniCtx(arg any, client *sdk.CloseApproachDataSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CloseApproachDataSDK); is {
			client = live
